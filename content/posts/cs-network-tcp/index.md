...
- 신뢰성 있는 연결형 서비스 제공
- 포트 번호를 이용한 다중화, 순서 제어, 오류 제어, 흐름 제어, 혼잡 제어 기능 제공
- 애플리케이션에는 메시지 경계를 보존하지 않는 연속된 바이트 스트림을 제공
...
![3way.png](image-001-optimized-image.webp)
...
![4way.png](image-002-optimized-image.webp)
...
2. 마지막 ACK 손실에 대응
    1. 클라이언트가 보낸 마지막 ACK가 손실되면 서버가 FIN을 다시 전송할 수 있다. TIME_WAIT 동안 클라이언트가 이 FIN을 받고 ACK를 재전송할 수 있다.

...
- 오버플로우 방지를 위해서는 `mid`를 계산할 때, `(low + high) / 2`보다 `low + (high - low) / 2`를 사용하기
...
아래 구현에서 탐색 범위는 반열린 구간 `[low, high)`이다. 배열 전체를 탐색하려면 `low = 0`, `high = 배열의 길이`로 호출한다. 이 규약을 사용하면 찾는 값이 모든 원소보다 클 때 배열의 길이를 삽입 위치로 반환할 수 있다.
...
```c
count = upper_bound(arr, 0, length, target)
      - lower_bound(arr, 0, length, target);
```
...
Binary Search를 구현할 때 가장 많이 발생하는 실수는 종료 조건(`low < high`, `low <= high`)과 `low`, `high` 갱신 방식을 서로 다른 구간 규약과 섞어 쓰는 것이다. 이때 off-by-one 오류가 발생하기 쉽다.

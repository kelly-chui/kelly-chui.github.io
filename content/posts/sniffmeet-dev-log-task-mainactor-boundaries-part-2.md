...
`didTabAcceptButton`과 `saveMateInfo`를 따라가 보니 유스케이스 실행을 위해 `Task`가 존재했고, Combine의 `sink` 클로저 안에서도 `Task`를 생성하고 있었다. Interactor에서 태스크를 묶으면 저장 완료를 알리는 메서드가 따로 필요해 보였다.

Presenter 메서드는 `async`로 바꾸고, `View`의 `sink`에서는 `Task`를 통해 호출하도록 두었다.
...
`didFetchUserInfo`에서는 `walkRequest` 생성 부분을 밖으로 옮겨도 가능했다. `didFetchUserInfo`의 실행 위치는 호출부를 함께 확인해야 하지만, `showRequestDetail`은 UI 작업이라 메인에서 돌아가야 해서 `@MainActor`를 붙였다.
...
순서가 중요했지만, 아래 코드에서는 `Task`를 시작한 직후 `didSendWalkRespond()`가 호출되어 요청 완료보다 성공 처리가 먼저 실행될 수 있었다. `respondWalkRequest`의 경우 `walkNoti`까지는 밖으로 분리하고, 성공 처리는 요청이 끝난 뒤 호출해야 했다.
...
비동기 처리보다는 `sleep`을 위한 `Task`에 가까웠다. `Task.sleep`은 특정 액터에서 실행되는 작업이 아니라 현재 태스크를 일시 중단한다.

이후 호출하는 `scrollRectToVisible`은 `UIScrollView`의 UI 작업이라 메인 액터에서 처리해야 했다.
...
`submitButton` 바인딩은 `debounce` 이후의 이벤트가 메인 런 루프에서 전달되도록 `RunLoop.main`을 사용했다.
...
`MPCManager`에서는 `MainActor` 제거 테스트가 필요했고, UI를 갱신하는 바인딩에는 `RunLoop.main`을 지정할 필요가 있었다. 다만 `MainActor`의 액터 격리와 `RunLoop.main`의 이벤트 전달 스케줄링은 서로 다른 역할이다.

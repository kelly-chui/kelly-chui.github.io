...
`UIWindow`의 루트 뷰 컨트롤러를 결정하는 흐름은 `@MainActor`로 두고, 세션 복원은 백그라운드에서 처리했다. 
...
등록 흐름은 더 고민이 많이 필요한 부분이었다. 4개의 태스크가 한 줄로 이어지면서 딜레이가 생겼고, 멀티스레드처럼 분리되어 동작하지는 않았다. 순서를 유지해야 해서 무작정 쪼개기도 어려웠다.

태스크를 조각내는 것도 가능해 보여서 태스크 그룹으로 나누는 방향도 떠올랐다. 다만 등록 후 다음 화면을 언제 보여줄지, 그리고 어느 흐름을 기준으로 `saveUserInfo`를 호출할지도 함께 정해야 했다. 최종적으로는 두 작업이 모두 끝난 뒤 `saveUserInfo`를 호출하는 방향으로 정리했다.

![image.png](/images/sniffmeet-study-log-async-task/image-001.png)
...
`requestMateList`는 현재 `MainActor`를 사용하고 있었는데, `alert.show` 때문에 UI와 연동이 필요한 것 아닐까 생각했다. 

확인 결과 `MainActor`가 없어도 잘 동작했고, `requestProfileImage`도 동일하게 `MainActor` 없이 동작했다.

다만 `output.mate` 쪽에는 `reloadData`가 있고, `output.profileImage` 데이터는 `reloadRow`만 main thread에서 사용하고 있어서 여기서는 메소드를 분리하는 방향이 맞아 보였다. 실제로 `MainActor`를 아예 빼는 쪽은 스레드 문제로 이어졌다.
...
테이블 뷰 각 셀이 로드될 때마다 요청이 발생해서 이미지 로드에 시간차가 생겼다. 요청을 전부 보내고 로딩은 한 번에 처리하는 방식으로 개선할 수 있겠다고 봤고, 이미지를 요청하는 역할을 따로 맡길 타입이 필요한지도 고민했다.

결론적으로는 한 번에 요청해 두고, 셀은 일단 그린 뒤 이미지 응답이 오면 셀 리로드가 일어나는 방식으로 정리했다. 이미 Combine이 있는 구조라서 더 자연스러웠다.
